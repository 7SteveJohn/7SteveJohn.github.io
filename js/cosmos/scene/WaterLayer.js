...
const FRAG = /* glsl */`
  ...
  uniform float uTime, uBass, uRippleStrength, uWaveSpeed;
  ...
  void main() {
    ...
    float mouseWave = sin(d * 45.0 - uTime * 6.0) * exp(-d * 6.0) * uRippleStrength;
    vec2 distort = vec2(w * 0.004 + mouseWave * 0.018, mouseWave * 0.03 + w * 0.002);
    ...
    float refStrength = (0.22 + uBass * 0.25) * smoothstep(0.0, 0.9, uv.y);
...
export class WaterLayer {
  ...
  setMouseUV(uv, active) {
    ...
    if (active) this.ripple = Math.min(1, this.ripple + this.cfg.water.rippleBoost);
  }
  update(t, bass) {
