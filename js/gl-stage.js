/* ============================================================
   ...
   · 让位接线（星河背景在场时）：首屏整体 dim 0.34 —— 环境件（尘埃/光环/背光/光晕）
     与画布透明度一起收，产品实体不吃这一项（首屏它还在 park 附近，morph≈0 自然看不见）。
   ============================================================ */
...
if (HOST) {
  ...
  function boot() {
    ...
    const halo = (function () {
      ...
      return {
        ...
        update(t, dt, level, sv, vis, dim) {
          ...
          if (!p.visible) return;
          mat.opacity = Math.min(0.85, level * 0.75) * vis * dim;
    ...
    /* ============================================================
       ...
       ============================================================ */
    const STOP_DEF = [
      { sel: '#home', obj: 'core', dim: 0.34, pos: [0, 2.5, 8.4], look: [0, 1.45, 0], fov: 40 },
    ...
    function buildStops() {
      ...
      for (const d of STOP_DEF) {
        ...
        if (d.items) {
          ...
          if (!blocks.length) stops.push({ obj: d.obj, dim: d.dim, y0: y0, y1: y0 + h, pos: d.pos, look: d.look, fov: d.fov });
        } else {
          stops.push({ obj: d.obj, dim: d.dim, y0: y0, y1: y0 + h, pos: d.pos, look: d.look, fov: d.fov });
        }
      }
...
try {        const dt = Math.min(0.05, clock.getDelta());
        ...
        // 「让位强度」：首屏 0.34（3D 退居背景，把画面交回星河背景），滚出首屏后平滑回到 1。
        // 被它乘上的只有环境件（尘埃 / 光环 / 背光 / 光晕）与画布透明度，产品实体不吃这一项。
        ...
        glow.material.opacity = Math.min(1, Math.max(0, anyOn - 0.25) * 0.85 * (1 + Math.min(0.7, Math.abs(sv) * 0.3))) * dim;
        ...
        back.intensity = (26 + Math.sin(t * 1.25) * 4 + Math.min(14, Math.abs(sv) * 6)) * dim;
        ...
        /* 滚出叙事区 → 画布淡出，交棒给正文；首屏让位时整体只留三成多，把画面交回星河背景 */
