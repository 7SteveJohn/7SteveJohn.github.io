...
原生 C# / WinForms 单文件，便携版双击就能跑，也有安装版；卸载就是删文件夹或走系统卸载，不残留。细节都在程序里的「说明」页。
  ...
  },
  {
    ...
    category: "desktop",
    categoryName: "桌面应用",
    description: "自用的完整版：系统优化 + 帧生成 + 游戏联动 + 实时监控，进游戏自动切状态、退出自动还原，改动都能一键还原。分享版（上一张卡）只保留了帧生成部分。",
    tags: ["C#", "WinForms", "游戏联动", "实时监控", "便携 + 安装版"],
    image: "assets/images/cover-fluxion.webp",
    featured: false,
    version: "v1.6.7",
    updated: "2026-09-21",
    requires: "Windows 10/11 · NVIDIA RTX 20 系及以上",
    details: `
### 💡 这是什么
我的自用主力工具：GameBoost（系统优化）和 DLSSG-Tool（帧生成）合并重构后的完整版——仪表盘、性能优化、帧生成、游戏库、实时监控，七个页面一个不缺。

完整版自用，暂不公开下载；想用帧生成功能的，用上一张卡片的分享版就行，功能是同一套。

### ⚡ 和分享版的区别
- **性能优化**：电源 / 调度 / GPU / 网络 / 内存一键优化，全部可一键还原
- **游戏联动**：进游戏自动切到游戏状态、退出自动还原，办公零影响
- **实时监控**：CPU / GPU / 内存负载与温度，nvlddmkm 崩溃取证
- **仪表盘**：系统健康度一屏看完，一键优化 / 体检 / 恢复备份直达

### 🎮 帧生成
与分享版同一套方案（dlssg_for_sm86 的 DLL 代理，AI 模型仍是 NVIDIA 原厂）：游戏库扫描、DX12 模式启动、运行诊断，都是现成的。

### 说明
完整版要动电源、注册表、网卡这些系统设置，风险面比分享版大，我还没整理成能放心发给别人的形态；什么时候整理好了再放出来。
  `
  }
];
